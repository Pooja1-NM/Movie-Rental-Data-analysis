...
select first_name,count(*) as 'Count of Repeated First Names' 
...
select first_name,count(*) as 'Count of Repeated First Names' from actor 
group by first_name  having count(*)=1  order by 2 desc;

select count(first_name) from view1;
-- Above Query Displays Count of Unique first name , There are 76 Unique first name of actors.

select first_name from view1 order by first_name asc;
...
select last_name,count(*) as 'Count of Repeated Last Names' 
from actor 
group by last_name 
order by 2 desc;
...
select rating,max(rental_rate) as 'Maximum Rental Rate',min(rental_rate) as 'Minimum Rental Rate',
avg(rental_rate) as 'Average Rental Rate' from film 
group by rating
...
select name,count(*) as 'Number of Movies' from category 
